...
from biocypher_cli.modules.sample_preparation import _normalize_rnacentral_rfam
...
def _urlopen_with_retry(src: str, retries: int = 1, delay: float = 1.0):
...
# Generalized file type handlers
FILE_TYPE_HANDLERS = {
    'vcf': {
        'detector': is_vcf_file,
        'sampler': lambda lines_iter, limit, no_header: _sample_vcf(lines_iter, limit),
        'post_processors': []
    },
    'uniprot': {
        'detector': is_uniprot_file,
        'sampler': lambda lines_iter, limit, no_header: sample_uniprot_records(lines_iter, limit),
        'post_processors': ['ensure_file_terminator'],
        'download_full': True  # Download full file before sampling
    },
    'default': {
        'detector': lambda path: True,  # Catch-all
        'sampler': lambda lines_iter, limit, no_header: _sample_default(lines_iter, limit, no_header),
        'post_processors': []
    }
}


def _sample_vcf(lines_iter: Iterator[str], limit: int) -> Iterator[str]:
    ...
    for line in lines_iter:
        if line.startswith('#'):
            ...
            if headers:
                yield from headers
                headers = None  # Don't yield again
            if data_sampled < limit:
                yield line
                data_sampled += 1
            else:
                break
    # Continue sampling remaining data lines
    for line in lines_iter:
        if data_sampled >= limit:
            break
        yield line
...
def get_file_handler(file_type: str) -> dict:
    """Get the handler dict for a file type."""
    return FILE_TYPE_HANDLERS.get(file_type, FILE_TYPE_HANDLERS['default'])


def register_file_type(type_name: str, detector, sampler, post_processors=None, download_full=False):
    ...
    post_processors: list of post-processing function names
    download_full: bool, whether to download full file before sampling
    """
    FILE_TYPE_HANDLERS[type_name] = {
        'detector': detector,
        'sampler': sampler,
...
def sample_local_file(local_path: Path, out_path: Path, limit: int, no_header: bool) -> Tuple[int, Path]:
    ...
    with open_maybe_gzip(target_path, "wt") as out:
        ...
        for line in handler['sampler'](lines, limit, no_header):
            out.write(line)
            data_rows += 1  # For VCF and default, this counts headers too, but approximate
    
    # Post-processing
    for post_proc in handler['post_processors']:
        try:
            if post_proc == 'ensure_file_terminator':
...
def stream_sample_http(src: str, out_path: Path, limit: int, no_header: bool) -> Tuple[int, Path]:
    parsed = urlparse(src)
    ...
    if is_zip:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _urlopen_with_retry(src) as resp, tmp_zip.open("wb") as outb:
    ...
    with _urlopen_with_retry(src) as resp:
        if is_gz:
            reader = io.TextIOWrapper(gzip.GzipFile(fileobj=resp), encoding="utf-8", errors="replace")
        else:
    ...
    if is_tar:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if parsed.scheme in {"http", "https"}:
                with _urlopen_with_retry(args.input) as resp, open(archive_path, "wb") as out:
        ...
        try:
            if not looks_like_dir:
                _normalize_rnacentral_rfam(target_path)
        except Exception:
            pass
        print(f"Wrote {data_rows} rows to {target_path}")
    else:
        if parsed.scheme in {"http", "https"}:
            data_rows, target_path = stream_sample_http(args.input, out_path, args.limit, args.no_header)
            print(f"Wrote {data_rows} rows to {target_path}")
        else:
